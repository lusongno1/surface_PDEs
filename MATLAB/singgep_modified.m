...
num = 300;%need to be changed in different case
% compute eigenvalues and left and right eigenvector (supported since Matlab 2014a)
[X,D1] = eigs(A1,B1,num,'smallestabs');
[Y,D2] = eigs(A1.',B1.',num,'smallestabs');
D = D1;
%sort(diag(D1))-sort(diag(D2))
...
Z = [(1:num)' d s a b esort esort<sep s>sepinf]; 
