function [lambda,Z,d,X,Y,U,V,DA,DB] = singgeps(A,B,N,show,nrank,tau,sep,sepinf,DA,DB)
...
if nargin<5, nrank = rank(rand(class_t)*A+rand(class_t)*B); end
...
if nargin<6, tau = numeric_t(1e-2,class_t); end
if nargin<7, sep = sqrt(eps(class_t)); end   % criteria for regular eigenvalues
if nargin<8, sepinf = 100*eps(class_t); end  % criteria for finite regular eigenvalues
...
[X,D,Y] = eigs(A1,B1,N,'smallestabs');%eigs(A,1,'smallestabs')
