...
void SetupInterfaceMassP1HighQuadP1Surf (const MultiGridCL& mg, MatDescCL* matM, const VecDescCL& ls, const BndDataCL<>& lsetbnd, double alpha)
{
    //ScopeTimerCL timer( "SetupInterfaceMassP1");
    ...
    InterfaceCommonDataP1CL cdata( ls, lsetbnd);
    accus.push_back( &cdata);
    InterfaceMatrixAccuCL<LocalInterfaceMassP1CLHighQuadP1Surf, InterfaceCommonDataP1CL> accu( matM, LocalInterfaceMassP1CLHighQuadP1Surf( alpha), cdata);
    accus.push_back( &accu);
    const IdxDescCL* RowIdx= matM->RowIdx;
    accumulate( accus, mg, RowIdx->TriangLevel(), RowIdx->GetBndInfo());
        ...
        if (triangle.Intersects())   // We are at the phase boundary.
        {
            ...
            for(int i= 0; i < 4; ++i)  // assemble row Numb[i]
            {
                ...
                for(int j= 0; j < 4; ++j)
                {
                    ...
                    if (xnumc[j] == NoIdx || is_pos[i]!= is_pos[j])
                        continue;
...
void SetupLBP1HighQuadP1Surf(const MultiGridCL& mg, MatDescCL* mat, const VecDescCL& ls, const BndDataCL<>& lsetbnd, double D)
{
    //input: multigrid, result matrix, dicrete level set, boundary data
    //ScopeTimerCL timer( "SetuLBP1");
    ...
    InterfaceCommonDataP1CL cdata( ls, lsetbnd);//store some info of zero ls
    accus.push_back( &cdata);//push zero ls to accumulator
    InterfaceMatrixAccuCL<LocalLaplaceBeltramiP1CLHighQuadP1Surf, InterfaceCommonDataP1CL> accu( mat, LocalLaplaceBeltramiP1CLHighQuadP1Surf( D), cdata);
    accus.push_back( &accu);//push A,visc,zero level set to accumulator
    const IdxDescCL* RowIdx= mat->RowIdx;//row index info of A
    accumulate( accus, mg, RowIdx->TriangLevel(), RowIdx->GetBndInfo());//calculate intergrant by accumulate
...
{
    //triangle now is bary coordinates
    for (int i= 0; i < 4; ++i)
        q[i].assign( p1[i], triangle);//limit basis function to triangle on std unit
    Quad5_2DCL<> qf( t, triangle, f,time), r;//limit and map rhs to std unit
//The barycentric coordinates are with respect to T, even if a transformation of T was applied. So we can get correct f value.
    for (int i= 0; i < 4; ++i)
    {
        if (Numb[i] == NoIdx)//NoIdx means discarded dof
            continue;
...
/*
 /// P1 with high quad modified 4.9
void SetupInterfaceRhsP1OnTriangleHighQuad (const LocalP1CL<> p1[4],
    Quad5_2DCL<> q[4],VectorCL& v, const IdxT Numb[4],
    const TetraCL& t, const BaryCoordCL triangle[3], double det,
    instat_scalar_fun_ptr f,double time=0)
{
...
//    for (int i= 0; i < 4; ++i)
//        q[i].assign( p1[i], triangle);//limit basis to tri and map basis on tri to std unit
...
//    }
    ...
    DROPS_FOR_TRIANG_CONST_TETRA( mg, lvl, it)//we need to find tetrahedron which contain useful triangles, most is useless
    {
        ...
        if (triangle.Intersects())   // We are at the phase boundary.
        {
            //(*it).GetVertex(3)->GetCoord()
            GetLocalNumbP1NoBnd( num, *it, *v->RowIdx);//num map four vertexes of tet to dof index of b

            for (int ch= 0; ch < 8; ++ch)//patch of triangle. Consider triangle is not triangle, break it into piece tris.
            {
                triangle.ComputeForChild( ch);//only when ch==7, it is vivid
                for (int tri= 0; tri < triangle.GetNumTriangles(); ++tri)
...
//                                  const VecDescCL& ls, const BndDataCL<>& lsetbnd, instat_scalar_fun_ptr f, double t)
//{
//    void lsFun(double x, double y, double z, double *value);//declare level set function x^2+y^2+z^2-1
//    void lsGrad(double x, double y, double z, double *grad);//declare gradient of level set function
//
//    const IdxT num_unks= v->RowIdx->NumUnknowns();//unknown number count, that vertexes name of tetra which intersect with bnd
//    const Uint lvl = v->GetLevel();//lvl is refine level
//
...
//
//
//    for(auto it=mg.GetTriangTetraBegin(lvl); it!=mg.GetTriangTetraEnd(lvl); ++it)//go through tetra
//    {
//        triangle.Init( *it, ls, lsetbnd);//init the zero level set of tetra it, lsetbnd is useless here
//        if (!triangle.Intersects())  continue;
//        //GetLocalNumbP1NoBnd( num, *it, *v->RowIdx);//num map four vertexes of tet to dof index of b
//        //find four vertexes of tet
//        const Uint sys= (*v->RowIdx).GetIdx();
//
//        for (Uint i= 0; i < 4; ++i)
//        {
...
//            if (num[iG] == NoIdx)
//                continue;
//            //auto vtx = (*itG).GetVertex(iG);
//            //TriangTetraCL tetG = (*it);
//            int n = phgQuadInterface2(
//                        lsFun,		/* the level set function */
//                        2,		/* polynomial order of the level set function */
//                        lsGrad,	/* the gradient of the level set function */
...
public:
    ...
    {
        return new StokesIFAccumulator_P1P1CL ( *this);
    }
...
void StokesIFAccumulator_P1P1CL::update_global_system ()
{
    ...
    for(int i= 0; i < 4; ++i)
    {
        ...
        if (ii==NoIdx || !(n.WithUnknowns( i)))
            continue;
        for(int j=0; j < 4; ++j)
        {
            const IdxT jj= n.num[j];
            if (jj==NoIdx || !(n.WithUnknowns( j)))
                continue;
            for (int k=0; k<3; ++k)
            {
                for (int l=0; l<3; ++l)
        ...
        for (int j=0; j < 4; ++j)
        {
            if (nScalar.num[j]==NoIdx || !(nScalar.WithUnknowns( j)))
                continue;
            for (int k=0; k<3; ++k)
            {
    ...
    for(int i= 0; i < 4; ++i)
    {
        ...
        if (ii==NoIdx || !(nScalar.WithUnknowns( i)))
            continue;
        for(int j=0; j <4; ++j)
        {
            const IdxT jj= nScalar.num[j];
            if (jj==NoIdx || !(nScalar.WithUnknowns( j)))
                continue;
...
public:
    ...
    TetraAccumulatorCL* clone (int /*tid*/)
    {
        return new NavierStokesIFAccumulator_P1P1CL ( *this);
    }
...
public:
    ...
    TetraAccumulatorCL* clone (int /*tid*/)
    {
        return new StokesIFAccumulator_P1P2CL ( *this);
    }
...
public:
    ...
    TetraAccumulatorCL* clone (int /*tid*/)
    {
        return new StokesIFAccumulator_P2P1CL ( *this);
    }
};
...
public:
    ...
    TetraAccumulatorCL* clone (int /*tid*/)
    {
        return new StokesIFAccumulator_P2P2CL ( *this);
    }
};
...
public:
    ...
    TetraAccumulatorCL* clone (int /*tid*/)
    {
        return new CahnHilliardIFAccumulator_P1P1CL ( *this);
    }
